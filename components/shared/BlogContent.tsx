...
const isSafeInternalLink = (href: string) =>
  href.startsWith("/") && !href.startsWith("//");
const isSafeExternalLink = (href: string) =>
  ...
  switch (block.type) {
    case "paragraph":
      return (
        <p
          key={`paragraph-${index}`}
          className="mt-6 text-left text-lg leading-8 text-slate-700 first:mt-0 sm:text-justify sm:leading-8 sm:[text-wrap:pretty] hyphens-auto"
        >
    ...
    case "h2":
      return (
        <h2
          key={`h2-${index}`}
          className="mt-12 text-2xl font-black tracking-[-0.03em] text-slate-900 sm:text-3xl"
        >
          {renderInlineNodes(block.content)}
        </h2>
      );
    case "h3":
      return (
        <h3
          key={`h3-${index}`}
          className="mt-8 text-xl font-black tracking-[-0.02em] text-slate-900 sm:text-2xl"
        >
          {renderInlineNodes(block.content)}
        </h3>
      );
    case "ul":
      return (
        <ul
          key={`ul-${index}`}
          className="mt-6 list-disc space-y-2 pl-6 text-lg leading-8 text-slate-700"
        >
    ...
    case "ol":
      return (
        <ol
          key={`ol-${index}`}
          className="mt-6 list-decimal space-y-2 pl-6 text-lg leading-8 text-slate-700"
        >
          {block.items.map((item, itemIndex) => (
    ...
    case "blockquote":
      return (
        <blockquote
          ...
          className="mt-8 whitespace-pre-line border-l-4 border-purple-600 bg-purple-50 p-5 text-lg italic leading-8 text-slate-700"
        >
          {renderInlineNodes(block.content)}
        </blockquote>
      );
