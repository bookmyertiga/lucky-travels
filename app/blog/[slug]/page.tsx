      ...
      <main className="min-h-screen bg-white py-6 sm:py-10">
        <article className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 force-justify">
          ...
          <nav aria-label="Breadcrumb navigation" className="mb-6">
            <Link
              href="/blog"
              className="inline-flex items-center gap-2 text-sm font-bold text-purple-700 hover:text-purple-900 transition"
            >
              <ArrowLeft size={16} /> Back to Travel Blog
            </Link>
          </nav>
          ...
          <header className="border-b border-slate-100 pb-8">
            <div className="flex flex-wrap items-center gap-2.5 text-xs font-bold uppercase tracking-wider text-slate-500">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-purple-100 px-3 py-1 text-purple-800">
                <Calendar size={13} /> {post.dateModified ? `Updated ${post.dateModified}` : post.date}
              </span>
              {post.author && (
              ...
              <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-amber-900">
                <Sparkles size={13} /> Verified Guide
              </span>
            </div>
            ...
            <p className="mt-4 text-base leading-relaxed text-slate-600 sm:text-lg justify-block">
              {post.excerpt}
            </p>
          ...
          <div className="pt-8 space-y-6 text-slate-700">
              ...
              switch (block.type) {
                ...
                case "h2":
                  return (
                    <h2
                      key={idx}
                      className="pt-8 text-xl sm:text-2xl font-black tracking-tight text-[#090f2f] border-t border-slate-100 leading-snug text-left"
                    >
                      {renderRichText(block.content)}
                    </h2>
                  );
                case "h3":
                  return (
                    <h3
                      key={idx}
                      className="pt-3 text-lg sm:text-xl font-black tracking-tight text-[#090f2f] leading-snug text-left"
                    >
                      {renderRichText(block.content)}
                    </h3>
                  );
                case "ul":
                  return (
                    <ul key={idx} className="my-5 space-y-3.5 pl-1 sm:pl-2">
                ...
                case "ol":
                  return (
                    <ol key={idx} className="my-5 space-y-3.5 pl-1 sm:pl-2">
                      {block.items.map((item, i) => (
                        <li key={i} className="flex items-start gap-3">
                ...
                case "blockquote":
                  return (
                    <blockquote
                      key={idx}
                      className="my-6 rounded-2xl border-l-4 border-amber-400 bg-amber-50/70 p-5 italic text-slate-800 shadow-sm text-base justify-block"
                    >
                      {renderRichText(block.content)}
                    </blockquote>
                  );
                case "image":
                  return (
                    <figure
                      key={idx}
                      className="my-8 overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 shadow-sm"
                    >
                      {block.href ? (
            ...
            {post.faqs && post.faqs.length > 0 && (
              <section className="mt-12 border-t border-slate-100 pt-8" aria-labelledby="faq-title">
                <h2 id="faq-title" className="text-xl sm:text-2xl font-black text-[#090f2f] text-left">
                  Frequently Asked Questions
                </h2>
                <div className="mt-4 divide-y divide-slate-100 rounded-2xl border border-slate-100 bg-slate-50/60 px-5 sm:px-6">
            ...
            <div className="mt-12 rounded-3xl bg-gradient-to-br from-[#080d2b] to-[#24105f] p-6 sm:p-8 text-white shadow-md">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                <a
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-xs sm:text-sm font-black text-purple-900 shadow hover:bg-slate-100 transition"
                >
                  <Phone size={16} /> Call Now: +91 {SITE.phone}
                </a>
                <a
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-green-600 px-5 py-3 text-xs sm:text-sm font-black text-white shadow hover:bg-green-700 transition"
                >
